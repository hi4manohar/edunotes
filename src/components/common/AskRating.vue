  ...
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent max-width="290">
      <v-card>
        ...
        <v-card-text
          >If you enjoy using <strong>Edunotes</strong>, please take a few
          seconds to rate 5 star on Playstore. It really helps!</v-card-text
        >
        ...
        <v-card-actions class="justify-space-between">
          ...
          <a :href="marketUrl" target="_blank">
            <v-btn color="primary" text @click="openRatingUrl()">
              Rate Now
            </v-btn>
          </a>
        </v-card-actions>
...
export default {
  ...
  mounted() {
    ...
    try {
      ...
      if (!configs.installation_date) return false;

      if (
        !configs.rateAction &&
        momentFilter.diffTodayDate(configs.installation_date) >= 2
      ) {
